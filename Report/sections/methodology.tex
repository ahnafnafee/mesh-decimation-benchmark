...
\begin{itemize}
    \item \textbf{Execution Time}: CPU time recorded in seconds. We measured per-run CPU time using \texttt{time.process\_time\_ns()}, which is less sensitive to interference from other user-level or kernel processes than wall-clock timing in a multi-programmed environment. Each condition was executed once as a warm-up and then repeated 5 times; the reported value is the mean of these 5 measured repetitions.
...
We measured execution time using \texttt{time.process\_time\_ns()} (CPU time). This reduces sensitivity to unpredictable OS scheduling effects and background activity that can distort wall-clock measurements in a multi-programmed environment. We still include a warm-up run (excluded from measurement) primarily to stabilize caches and allocation behavior before the 5 measured repetitions.
